...
async def execute_issues(
    ...
    for issue in issues:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"The AI responded with an answer that is a number but does not correspond to an option"
                              f"\nDismissing issue...")
                selected_option = -1
        except ValueError:
            ...
            for option in issue.options:
                ...
                if selected_option in option.text:
                    ...
                    break
            if not type(selected_option) == int:
                logging.error(f"Unable to parse AI response to issue"
                              f"\nDismissing issue...")
                selected_option = -1
    ...
    await huggingface_session.close()
    return execute
...
async def ns_ai_bot(
    ...
    while True:
        ...
        next_issue_time = await time_to_next_issue(nation, issues[1])
        logging.info(
